...
router.post('/outbound-webrtc', express.json(), async (req, res) => {
  ...
  try {
    ...
    if (event_type === 'call.initiated' && direction === 'outgoing' && state === 'parked') {
      broadcast('WebRTC_OutboundCCID', req.body.data.payload.call_control_id, req.env);
      console.log("WEBRTC OUTBOUND CCID", req.body.data.payload.call_control_id)
      req.prisma.voice.create({
        data: {
...
router.post('/outbound-webrtc-bridge', express.json(), async (req, res) => {
  ...
  if (event_type === 'call.answered') {
    broadcast('OutboundCCID', req.body.data.payload.call_control_id, req.env);
    try {
  ...
  if (event_type === 'call.bridged') {
    ...
    try {
      const voiceRow = await req.prisma.voice.findFirst({ where: { queue_uuid: callControlId_Bridge } });
      ...
      // Update statuses
      await req.prisma.callLeg.updateMany({ where: { call_control_id: callControl }, data: { status: 'active' } });
      await req.prisma.callSession.updateMany({ where: { sessionKey: callControlId_Bridge }, data: { status: 'active' } });
  ...
  if (event_type === 'call.hangup') {
    try {
      ...
      await req.prisma.callLeg.updateMany({
        where: { call_control_id: callControl }, data: {
          status: 'ended',
          end_time: new Date(),
      ...
      if (callControlId_Bridge) {
        const customerLeg = await req.prisma.callLeg.findFirst({ where: { sessionKey: callControlId_Bridge, leg_type: 'customer' } });
        if (customerLeg && customerLeg.status === 'ended') {
          await req.prisma.callSession.updateMany({ where: { sessionKey: callControlId_Bridge }, data: { status: 'ended', ended_at: new Date() } });
        }
      }
    } catch (e) {
  ...
  if (event_type === 'call.hangup' && hangup_source === "callee" && clientState !== Buffer.from("Transfer").toString('base64')) {
    ...
    try {
      ...
      broadcast('OutboundCCID', null, req.env);
...
router.post('/outbound-call-webhook', express.json(), async (req, res) => {
  ...
  try {
    ...
    if (event_type === 'call.answered' && stage === 'agent') {
      ...
      // Update database
      await req.prisma.voice.updateMany({ where: { queue_uuid: pstnCallControlId }, data: { status: 'bridged' } });
    ...
    if (event_type === 'call.hangup') {
      ...
      // Update database
      await req.prisma.voice.updateMany({ where: { queue_uuid: callControlId }, data: { status: 'ended' } });
